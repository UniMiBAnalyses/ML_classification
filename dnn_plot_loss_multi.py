...
class PlotLosses(keras.callbacks.Callback):
    def __init__(self, model, data, batch_mode=False):
        ...
        self.y_train = data["y_train"][:,0]
        self.y_test = data["y_val"][:,0]
    ...
    def performance_save(self, logs):
        ...
        self.pred_test_temp = self.model.predict(self.X_test, batch_size=2048)[:,0]
        auc_w_test = roc_auc_score(self.y_test, self.pred_test_temp, sample_weight=self.W_test)
        self.auc_test.append(auc_w_test)
        self.pred_train_temp = self.model.predict(self.X_train, batch_size=2048)[:,0]
